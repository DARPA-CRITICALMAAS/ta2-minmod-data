...
### Building the image:
```
docker build -t container_minmod_ta2_deploy .
```

### Running the container
To generate the file in the <hostmachine_volume>, run the following command:
```
docker run --env TTL=<ttl_path> container_minmod_ta2_deploy
```
For example:
```
docker run --env TTL=/tmp/final.ttl container_minmod_ta2_deploy
```
Make sure that:
